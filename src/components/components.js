...
function AboutContent(){
  return (
    <div>
      ...
      <p> <strong> Current/Final Allocation </strong> - It is generally recommended to shift the asset allocation of a retirement portfolio from a higher risk portfolio to a lower risk portfolio over time, to allow for better stability in retirement. Equity investments are generally considered higher risk (and are therefore expected to generate a higher return) than fixed income investments. This calculator allows for accounting for this so called <a herf='https://www.investopedia.com/terms/g/glide-path.asp'>Glide Path</a>. This calculator assumes that asset allocations will change on a straight line basis from the beginning allocation to the ending allocation between the current date and the retirement date. </p>
